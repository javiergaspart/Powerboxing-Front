...
Future<void> fetchAvailableSessions() async {
  ...
  for (var session in decodedData) {
    if (session["date"] == null || session["time"] == null) continue;
  ...
  // ✅ Sort times in ascending order for each date
  availableSessions.forEach((date, times) {
    times.sort((a, b) {
      int timeA = int.parse(a.replaceAll(':', '')); // Convert "8:00" → 800
      int timeB = int.parse(b.replaceAll(':', '')); // Convert "10:00" → 1000
